...
export default function AccountAccessPanel() {
  ...
  return <div className="settings-section-stack settings-access space-y-6">
    <header className="flex flex-col gap-2">
      ...
      <h1 className="text-3xl font-semibold text-brand-forest-800 sm:text-4xl">Share your story with the right people</h1>
    ...
    <section className="rounded-2xl border border-brand-terracotta-500/20 bg-brand-paper-100/60 p-5 shadow-sm">
      ...
      <h2 className="mt-1 text-xl font-semibold text-brand-forest-800">Choose the story you’re working on</h2>
      ...
      <div className="mt-4 grid gap-2 sm:grid-cols-2">
        {ownedSites.map(site => (
          <button
            ...
            className={`rounded-xl border p-3 text-left transition ${Number(site.id) === Number(activeHouseholdId) ? 'border-brand-terracotta-500 bg-white text-brand-forest-800 shadow-sm' : 'border-gray-200 bg-white/70 text-gray-700 hover:border-brand-terracotta-500/50'}`}
      ...
      {active && ['owner', 'admin'].includes(active.role) && <form className="mt-5 border-t border-brand-terracotta-500/10 pt-4" onSubmit={renameSite}>
        <label className="block text-sm font-semibold text-gray-700" htmlFor="memory-site-name">Site name</label>
        <div className="mt-2 flex flex-col gap-2 sm:flex-row"><input id="memory-site-name" value={rename} onChange={event => setRename(event.target.value)} minLength={2} maxLength={80} required className="min-w-0 flex-1 rounded-lg border border-gray-200 px-3 py-2" /><button className="rounded-lg bg-brand-terracotta-500 px-4 py-2 text-sm font-semibold text-white disabled:opacity-60" disabled={working === 'rename'}>{working === 'rename' ? 'Saving…' : 'Rename site'}</button></div>
      </form>}
    </section>

    <section className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
      <p className="memory-eyebrow">Your account</p>
      <h2 className="mt-1 text-xl font-semibold text-brand-forest-800">Secure email identity</h2>
    ...
    <section className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
      ...
      <h2 className="mt-1 text-xl font-semibold text-brand-forest-800">{active?.name || 'This memory site'}</h2>
      ...
      {['owner', 'admin'].includes(access.role) && <form className="mt-4 border-t border-gray-100 pt-4" onSubmit={event => { event.preventDefault(); run('invite', async () => { const result = await api.inviteHouseholdMember(inviteEmail); setInviteEmail(''); setMessage(result.message); await loadAccess(); }); }}>
        ...
        <div className="mt-2 flex flex-col gap-2 sm:flex-row">
          ...
          <button className="rounded-lg border border-brand-terracotta-500 px-4 py-2 text-sm font-semibold text-brand-terracotta-500 disabled:cursor-not-allowed disabled:opacity-50" disabled={!canInviteMember || working === 'invite'}>{working === 'invite' ? 'Sending…' : 'Send invitation'}</button>
        </div>
    ...
    <section className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
      ...
      <h2 className="mt-1 text-xl font-semibold text-brand-forest-800">Control where you are signed in</h2>
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        <button type="button" onClick={() => run('sessions', async () => { const result = await api.revokeOtherSessions(); setMessage(result.message); })} disabled={working === 'sessions'} className="rounded-lg border border-brand-terracotta-500 px-4 py-2 text-sm font-semibold text-brand-terracotta-500 disabled:opacity-60">
    ...
    <section className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
      ...
      <h2 className="mt-1 text-xl font-semibold text-brand-forest-800">Start another memory site</h2>
      ...
      <form className="mt-4 flex flex-col gap-2 sm:flex-row" onSubmit={event => { event.preventDefault(); run('site', async () => { await api.createHousehold(siteName); window.location.href = '/'; }); }}>
        ...
        <button className="rounded-lg border border-brand-terracotta-500 px-4 py-2 text-sm font-semibold text-brand-terracotta-500 disabled:opacity-60" disabled={working === 'site'}>{working === 'site' ? 'Creating…' : 'Create memory site'}</button>
      </form>
    </section>
