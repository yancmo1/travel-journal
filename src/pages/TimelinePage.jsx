      ...
      {onThisDay.length > 0 && (
        <section className="rounded-2xl border border-sunset-orange/20 bg-orange-50/70 p-5 shadow-sm">
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-2">
            {onThisDay.map(memory => <TimelineCard key={memory.id} memory={memory} />)}
          </div>
        </section>
      )}

      <section className="relative border-l-2 border-ocean-teal/20 pl-5 sm:pl-8">
        {timeline.map(memory => <TimelineCard key={memory.id} memory={memory} timeline />)}
...
function TimelineCard({ memory, timeline = false }) {
  const photo = memory.photos?.find(item => item.is_cover) || memory.photos?.[0];
  return (
    <article className="relative mb-5 rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
