...
The implementation source of truth is [`src/styles/brand-tokens.css`](../src/styles/brand-tokens.css).
...
- Use forest for navigation, headings, and calm authority.
- Use paper for the canvas and cards; avoid pure white as the dominant background.
- Reserve terracotta for actions, links, markers, and moments that need attention.
- Use brass sparingly for map and archival details.
- Maintain readable contrast for older users; never use a pale accent for essential text.

## Typography

- **Display:** Playfair Display, semibold, used for page titles, card titles, and meaningful numbers.
- **Body/UI:** DM Sans, regular through bold, used for navigation, helper copy, labels, and controls.
- Headlines are sentence case, conversational, and short.
- Uppercase kicker labels are small, letter-spaced, and used sparingly for hierarchy.
...
- Cards should reveal a photo first, then location and date. Metadata remains secondary.
- Maps sit inside a framed paper surround and should remain visually quiet beneath the memories.
...
- “Manage entities”
- “Analytics overview”
- Cold, technical, or overly clever labels.
...
- Keep map controls available, but do not make the map the only way to access a memory.

## Change workflow

1. Change a value in `src/styles/brand-tokens.css` first.
2. Use the variable in component CSS; do not introduce a new one-off hex value without a reason.
3. Update this book when the brand meaning, not just an implementation detail, changes.
4. Verify desktop, tablet, mobile, keyboard focus, and the production build.

## Asset

The approved stamp logo is `assets/postcards-of-us-stamp.png`. Use it as the primary brand mark in navigation and other identity-forward surfaces. Do not redraw it with CSS or replace it with a generic letter mark.
