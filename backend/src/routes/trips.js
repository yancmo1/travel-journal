  ...
  try {
    ...
    let sql = `
      SELECT t.*, 
        ...
        COALESCE(
          json_agg(DISTINCT jsonb_build_object('id', p.id, 'filename', p.filename, 'thumbnail_path', p.thumbnail_path)) 
...
router.post('/', async (req, res, next) => {
  try {
    const {
      locationName, latitude, longitude, country, state,
      startDate, endDate, tripType, notes, travelerIds
    } = req.body;
    ...
    `, [
      locationName, latitude, longitude, country, state,
      startDate, endDate || null, tripType, notes, homeDist, req.user.id
    ]);
...
router.put('/:id', async (req, res, next) => {
  try {
    ...
    const result = await query(`
      UPDATE trips SET
        location_name = $1, latitude = $2, longitude = $3, country = $4, state = $5,
        start_date = $6, end_date = $7, trip_type = $8, notes = $9, 
        home_distance_miles = $10, updated_at = NOW()
      WHERE id = $11
      RETURNING *
    `, [
      locationName, latitude, longitude, country, state,
      startDate, endDate || null, tripType, notes, homeDist, id
    ]);
