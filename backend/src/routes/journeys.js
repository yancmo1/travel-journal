      ...
      FROM (
        SELECT t.*,
          COALESCE((
            ...
            ) ORDER BY p.date_taken NULLS LAST, p.uploaded_at)
            FROM photos p
...
router.post('/', async (req, res, next) => {
  try {
    const { title, startDate, endDate, dateLabel, journeyType, summary, memoryIds = [] } = req.body;
    if (!title?.trim()) return res.status(400).json({ error: 'Journey name is required' });

    const result = await query(`
      INSERT INTO journeys (title, start_date, end_date, date_label, journey_type, summary, created_by)
      VALUES ($1, $2, $3, $4, $5, $6, $7)
      RETURNING *
    `, [
      title.trim(), startDate || null, endDate || null, dateLabel || null,
      journeyType || 'Other', summary || null, req.user.id
    ]);
  ...
  try {
    ...
    const result = await query(`
      UPDATE journeys SET
        ...
        journey_type = $5, summary = $6, updated_at = CURRENT_TIMESTAMP
      WHERE id = $7
      RETURNING id
    `, [
      title.trim(), startDate || null, endDate || null, dateLabel || null,
      journeyType || 'Other', summary || null, req.params.id
    ]);
