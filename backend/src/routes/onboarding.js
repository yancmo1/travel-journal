...
router.get('/', async (req, res, next) => {
  try {
    ...
    const [progress, user, memoryCount, journeyCount, peopleCount] = await Promise.all([
      query('SELECT home_skipped, people_skipped, memory_id, journey_id, welcome_seen, completed_at FROM onboarding_progress WHERE household_id = $1 AND user_id = $2', [householdId, req.user.id]),
    ...
    res.json({ home: { complete: homeComplete, skipped: Boolean(row.home_skipped) && !homeComplete }, people: { complete: peopleComplete, skipped: Boolean(row.people_skipped) && !peopleComplete }, memory: { complete: memoryComplete, id: row.memory_id || null }, journey: { complete: journeyComplete, id: row.journey_id || null }, welcomeSeen: Boolean(row.welcome_seen), completed: (Boolean(row.completed_at) && peopleComplete) || (homeComplete && peopleComplete && memoryComplete && journeyComplete) });
  } catch (err) { next(err); }
});
...
router.patch('/', async (req, res, next) => {
  try {
    ...
    if (!householdId || !['welcome', 'home', 'people', 'memory', 'journey'].includes(step)) return res.status(400).json({ error: 'Choose a valid onboarding step.' });
    ...
    await query(`
      ...
      ON CONFLICT (household_id, user_id) DO UPDATE SET
        home_skipped = CASE WHEN $3 THEN TRUE WHEN $8 = 'home' THEN FALSE ELSE onboarding_progress.home_skipped END,
        people_skipped = CASE WHEN $4 THEN TRUE WHEN $8 = 'people' THEN FALSE ELSE onboarding_progress.people_skipped END,
        ...
        completed_at = CASE WHEN $8 = 'journey' AND $6 IS NOT NULL AND $9 THEN CURRENT_TIMESTAMP WHEN NOT $9 THEN NULL ELSE onboarding_progress.completed_at END,
        updated_at = CURRENT_TIMESTAMP
    `, [householdId, req.user.id, homeSkipped, peopleSkipped, memoryId, journeyId, step === 'welcome', step, onboardingReady]);
