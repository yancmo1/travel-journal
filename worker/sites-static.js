  ...
  if (!onboardingTableReady.has(env.DB)) {
    onboardingTableReady.set(env.DB, env.DB.prepare(`
      CREATE TABLE IF NOT EXISTS onboarding_progress (
        ...
        refresher_hidden INTEGER NOT NULL DEFAULT 0,
        completed_at TEXT,
...
async function handleFetch(request, env, ctx) {
    ...
    if (url.pathname.startsWith('/api/')) {
      ...
      if (url.pathname === '/api/onboarding' && request.method === 'GET') {
        await ensureOnboardingTable(env);
        const row = await env.DB.prepare(`
          SELECT home_skipped, people_skipped, memory_id, journey_id, welcome_seen, refresher_hidden, completed_at
        ...
        return json({
          ...
          refresherHidden: Boolean(row?.refresher_hidden),
      ...
      if (url.pathname === '/api/onboarding' && request.method === 'PATCH') {
        ...
        const allowedSteps = new Set(['welcome', 'home', 'people', 'memory', 'journey', 'refresher']);
        ...
        const refresherHidden = step === 'refresher' && body?.status === 'hidden' ? 1 : 0;
        ...
        await env.DB.prepare(`
          ...
          ON CONFLICT(household_id, user_id) DO UPDATE SET
            ...
            refresher_hidden = CASE WHEN excluded.refresher_hidden = 1 THEN 1 ELSE onboarding_progress.refresher_hidden END,
            ...
            updated_at = CURRENT_TIMESTAMP
        `).bind(user.household_id, user.id, homeSkipped, peopleSkipped, memoryId, journeyId, step === 'welcome' ? 1 : 0, refresherHidden, step, step, step, journeyId, onboardingReady, onboardingReady).run();
